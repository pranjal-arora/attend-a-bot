    ...
    function capture() {
      if (
        ...
        if (!!fullDesc) {
          ...
          fullDesc.map((desc) => {
            //mark and label the face with the name and matric number of the student that have the closest L2 distance.
            const bestMatch = faceMatcher.findBestMatch(desc.descriptor);
            console.log(bestMatch);
            if (bestMatch._label != "unknown") {
              //store the respective attendance ID and the student ID into the database
              createTrxCallback({
                variables: {
