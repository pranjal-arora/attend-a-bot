...
// import react-webcam that help to retrieve the video frame from the webcam.
    ...
    function capture() {
      if (
        ...
        setPreviewImage(webcamRef.current.getScreenshot());
          //it will return BLOB value of the current frame
        ...
        // Draw mesh 
        //obtains the feature vectors, if the face is not detected in the first hand, then there will not be any feature vectors generated.
...
//we have set the infinite loop for the useEffect() that update the video frame for every 0.7 second.
  ...
  const handleSubmit = () => {
    addFacePhotoCallback({
      ...
      variables: {
        ...
        faceDescriptor: faceDescriptor.toString(), //parsing float32array to string
      },
    });
  };
